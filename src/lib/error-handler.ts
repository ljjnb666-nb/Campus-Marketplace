...
export function handleError(error: unknown, context: string): HandledError {
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    switch (error.code) {
      ...
      default:
        logger.error("数据库请求失败", context, { error, prismaCode: error.code });
        return { message: "操作失败，请稍后重试", statusCode: 500 };
    }
  }

  if (error instanceof Prisma.PrismaClientValidationError) {
    logger.error("数据库查询参数非法", context, { error });
    return { message: "操作失败，请稍后重试", statusCode: 500 };
  }

  logger.error("未处理异常", context, { error });
  return { message: "服务器内部错误，请稍后重试", statusCode: 500 };
}
