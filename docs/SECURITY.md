...
- E2E 浏览器安全回归（2026-08-30，Production Phase 2，详见 [E2E.md](E2E.md)）：
  - 私有资产 HTTP 边界四角色真实验证：上传者/订单参与方 200、ADMIN 200、无关用户 403、匿名 401（真实 `/api/assets/{id}/access` 请求）
  - 权限负例上浏览器门禁：普通用户访问 `/admin` 被重定向、跨用户打开商品编辑页 notFound、无关用户不可见他人订单
